#!/usr/bin/env bun
/** Seed BFME's required French locale in a large STORE WGB without rewriting 3 GB of ROM. */
import * as fs from "node:fs";

const archive = process.argv[2];
if (!archive) throw new Error("usage: bun tools/patch-bfme-registry-inplace.ts <game.wgb>");
...
try {
    ...
    let centralPos = tail.readUInt32LE(eocd + 16);
    let centralEntry = -1;
    let localOffset = -1;
    let storedSize = -1;
    ...
    for (let i = 0; i < entryCount; i++) {
        ...
        if (name.toString("utf8") === "registry.json") {
            if (header.readUInt16LE(10) !== 0) throw new Error("registry.json must use STORE compression");
            centralEntry = centralPos;
    ...
    if (centralEntry < 0) throw new Error("registry.json not found");

    const local = Buffer.alloc(30);
    fs.readSync(fd, local, 0, local.length, localOffset);
    if (local.readUInt32LE(0) !== 0x04034b50) throw new Error("bad registry local header");
    const dataOffset = localOffset + 30 + local.readUInt16LE(26) + local.readUInt16LE(28);
    const encoded = Buffer.from(JSON.stringify(seed), "utf8");
    if (encoded.length > storedSize) throw new Error(`registry seed needs ${encoded.length} bytes; slot has ${storedSize}`);
    ...
    fs.writeSync(fd, crcBytes, 0, 4, centralEntry + 16);
    fs.fsyncSync(fd);
    console.log(`Patched ${archive}: BFME Language=French (${encoded.length}/${storedSize} bytes), crc=0x${crc.toString(16)}`);
} finally {
    fs.closeSync(fd);
}
