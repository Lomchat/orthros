...
BottleShip executes **untrusted x86 guest code** (the games you load) entirely inside the
...
- **The emulator's memory model** (paged 4 GB address space, region permissions, copy-on-write
  overlays) isolates guests from BottleShip's own JS/WASM state, and games from one another.
...
BottleShip does not, and cannot, vet the behavior of arbitrary executables you supply.

## Supported versions

BottleShip is pre-1.0 and moves fast; security fixes land on the default branch. There are no
